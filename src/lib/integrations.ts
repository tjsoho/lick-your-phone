...
interface IntegrationContext {
  ...
  signerEmail: string
  totalCents: number
  lineItems: Array<{ serviceName: string; priceCents: number; billing: string }>
  documentUrl: string
  documentHash: string
}
...
async function sendClientEmail(ctx: IntegrationContext) {
  ...
  if (!resendApiKey) {
    console.log("[integrations] RESEND_API_KEY not set — skipping client email")
    return
  }
  ...
  try {
    const fromEmail = process.env.RESEND_FROM_EMAIL || "hello@lickyourphone.com.au"
    ...
    console.error("[integrations] Client email failed:", (err as Error).message)
  }
}
...
async function emitN8nWebhook(
  ...
  try {
    ...
    await failJob(jobId, (err as Error).message)
    console.error(`[integrations] n8n ${event} webhook failed:`, (err as Error).message)
  }
}
...
export async function onIntakeCompleted(
  ...
  const webhookUrl = process.env.N8N_WEBHOOK_URL
  if (!webhookUrl) {
    console.log("[integrations] N8N_WEBHOOK_URL not set — skipping intake_completed")
    return
  }
  ...
  try {
    ...
    await failJob(jobId, (err as Error).message)
    console.error("[integrations] intake_completed webhook failed:", (err as Error).message)
  }
}
